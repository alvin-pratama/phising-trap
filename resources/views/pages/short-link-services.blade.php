...
@section('title', 'Short Link Services')

@section('content')
<h1 class="text-2xl font-bold">Short Link Services</h1>
<p class="mt-2 text-gray-700">Atur konfigurasi sistem di sini.</p>

<!-- Card -->
<div class="bg-white shadow-md rounded-lg mt-6 p-6">
    <h2 class="text-xl font-semibold text-gray-800">Daftar Short Link Services</h2>
...
<script>
    ...
    function loadData(page = 1) {
        fetch(`/short-link-services/all?page=${page}`)
                ...
                data.data.data.forEach((service, index) => {
                    table.innerHTML += `
                        <tr id="row-${service.id}" class="border-b hover:bg-gray-100">
                            ...
                            <td class="border border-gray-300 px-4 py-2">${service.func}</td>
    ...
    function addService() {
        ...
        let func = document.getElementById("func").value.trim();
